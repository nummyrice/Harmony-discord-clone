...
export default function CreateServer({setCreateNewServer, setServerActive}) {
    ...
    const validate = () => {
        ...
        if (!serverName) validationErrors.push('name : You must name your server.');
        if (!imageUrl) validationErrors.push('image : Add a server image to make it your own.')
    ...
    const handleSubmit = async (e) => {
        ...
        if (JSON.stringify(imageUrl) === '{}') {
            console.log('hello')
            formData.append('image_url', 'https://i.ibb.co/QNxL2kj/Discord-Logo-Color.png')
        } else {
            console.log('goodbye')
    ...
    return (
        <>
            ...
            <div className={style.newServerContainer}>
                <form
                    className={style.newServerForm}
                    onSubmit={handleSubmit}
                >
                    <div
                        className={style.newServerCloseContainer}
                        onClick={() => setCreateNewServer(false)}
                    >
                        <svg
                            className={style.newServerX}
                            aria-hidden="false"
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                    ...
                    <div className={style.newServerContent}>
                        ...
                        <div className={style.newServerInputWrapper}>
                            <label className={style.newServerLabel}>
                                SERVER NAME<span
                                    className={style.newServerSpan}
                                >
                                {errors.length > 0 && errors.map(error => (
                                    error.includes('name')
                                )) ? errors.map(error => error.includes('name') ?
                                    ` - ${error.split(':')[1]}` : null) : null}
                                </span>
                            </label>
                            <input
                                className={style.newServerInputField}
                                value={serverName}
                    ...
                    <div className={style.newServerButtons}>
                        ...
                        <button
                            className={style.newServerSubmit}
                        >
                            Create
                        </button>
                    </div>
