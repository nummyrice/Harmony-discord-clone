...
export default function ChannelMessages() {
    ...
    const [ editMessageInput, setEditMessageInput ] = useState({id: undefined, value: ''});
    ...
    function messageSubmit(e) {
        ...
        if (messageInput !== '') {
            setMessageInput('');
            return dispatch(serverActions.postMessageThunk({content: messageInput, server_id:server.id, channel_id:channel.id}))
        }
    }
    ...
    function editMessageSubmit(e) {
        ...
        if (editMessageInput.value !== '') {
            ...
            return dispatch(serverActions.editMessageThunk({
    ...
    if (channel) {
        if (JSON.stringify(channel.messages) === '{}') {
            dispatch(serverActions.getMessagesThunk({ server_id: serverId, channel_id: channelId }))
        }
    };
    ...
    return (
        <div>
            <div className={style.channel_messages_container}>
                {server?.image_url && <img alt={'avatar'} className={style.server_image} src={server.image_url}/>}
                ...
                {messages && Object.values(messages).map((message) => {
                    return(
                        <div className={style.message_container} key={message.id}>
                            <img alt={'avatar'} className={style.owner_image}src={message.owner.image_url}></img>
                            ...
                            <>
                                <form onSubmit={editMessageSubmit}>
                                    <input className={style.edit_message_input_field}
                                    type="text"
                                    value={editMessageInput.value}
                                    onChange={(e) => setEditMessageInput({id: editMessageInput.id, value: e.target.value})}>
                                    </input>
                                    <div
                                    className={style.edit_message_cancel}
                                    onClick={() => setEditMessageInput({id: undefined, value: ''})}> Cancel
                                    </div>
                                </form>
                            </>
                            }
                            {(parseInt(displayDeleteConfirm, 10) === message.id) && <ConfirmDeleteModal setDisplayDeleteConfirm={setDisplayDeleteConfirm} message={message}/>}
                        </div>
                    )
                })}
            </div>
